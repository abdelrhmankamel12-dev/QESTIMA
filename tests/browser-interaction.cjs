...
async function main() {
  ...
  try {
    ...
    assert.equal(await page.locator('[data-action="toggle-navigator"]').evaluate(el => getComputedStyle(el).color), 'rgb(32, 37, 43)')
    const previewDescription = page.locator('input[data-boq-id][data-field="description"]').first()
    await previewDescription.fill('توريد وتركيب مواسير PPR قطر 25 مم شاملة الوصلات')
    await previewDescription.press('Tab')
