...
test("encrypted Project Package round-trips, verifies attachments and detects corruption", () => {
  ...
  try {
    ...
    const key = crypto.randomBytes(32); const created = createProjectPackage({ targetRoot: root, state: sampleState(), sourceAttachments: attachments, key, appVersion: "0.15.0" })
    ...
    const portable = createProjectPackage({ targetRoot: portableRoot, state: sampleState(), sourceAttachments: attachments, passphrase: "portable-passphrase", appVersion: "0.15.0" })
...
test("runtime staging can cross-target a Windows x64 runtime", () => {
  ...
  try {
    ...
    assert.equal(validateRuntimeCandidate(output, { expectedAppVersion: "0.15.0" }).ok, true)
  } finally { fs.rmSync(folder, { recursive: true, force: true }) }
})
...
test("release preflight simulates install, upgrade and rollback without creating an installer", () => {
  ...
  try {
    ...
    assert.equal(validateRuntimeCandidate(staging, { expectedAppVersion: "0.15.0" }).ok, true)
    const installDir = path.join(folder, "installed"); const first = installStagedRuntime({ staging, installDir, expectedAppVersion: "0.15.0" })
    ...
    const upgraded = upgradeStagedRuntime({ staging: upgradedStaging, installDir, expectedAppVersion: "0.15.0" })
