...
async function main() {
  ...
  try {
    ...
    await page.locator('#mfa:visible').waitFor(); await page.locator('#begin-mfa').click(); await page.locator('#mfa-secret').filter({hasText:/[A-Z2-7]{16,}/}).waitFor({state:'visible'})
    ...
    await page.locator('#users tr').filter({hasText:'مهندس تسعير'}).waitFor({state:'visible'})
    await page.locator('#show-progress').click(); await fill('#project-form',{name:'مشروع اختبار MEP'}); await submit('#project-form')
    await page.locator('#progress').filter({hasText:'مشروع اختبار MEP'}).waitFor({state:'visible'})
